...
class Buffer {
	...
	public function wp_head() {
		...
		$post_id = apply_filters( 'wpml_object_id', get_the_ID(), get_post_type(), true, apply_filters('wpml_default_language', NULL ) );

		if (is_singular()) {
			$_header = get_post_meta($post_id, '_wpda-builder-header', true);
			if (!empty($_header)) {
				$_header = apply_filters( 'wpml_object_id',$_header, 'elementor_library' ) ;
...
//					}
			...
			$_footer = get_post_meta($post_id, '_wpda-builder-footer', true);
			if (!empty($_footer)) {
				$_footer = apply_filters( 'wpml_object_id',$_footer, 'elementor_library' ) ;
			...
			if ($headers->have_posts()) {
				...
				foreach ($headers->posts as $_header_id) {
					if (function_exists('pll_get_post')) {
						$_header_id = pll_get_post($_header_id, pll_default_language());
					}
					$conditions = get_post_meta($_header_id, '_wpda-builder-conditions', true);
					$_header_id = apply_filters( 'wpml_object_id',$_header_id, 'elementor_library' ) ;
					try {
		...
		if (false === $footer) {
			...
			if ($footers->have_posts()) {
				...
				foreach ($footers->posts as $_footer_id) {
					if (function_exists('pll_get_post')) {
						$_footer_id = pll_get_post($_footer_id, pll_default_language());
					}
					$conditions = get_post_meta($_footer_id, '_wpda-builder-conditions', true);
					$_footer_id = apply_filters( 'wpml_object_id',$_footer_id, 'elementor_library' ) ;
					try {
						$conditions = json_decode($conditions, true);
						if (json_last_error() || !is_array($conditions)) {
							$conditions = $default_conditions;
				...
				if (false === $footer && count($all_footers)) {
					foreach ($all_footers as $footer_id) {
						$footer_id = apply_filters( 'wpml_object_id',$footer_id, 'elementor_library' ) ;
