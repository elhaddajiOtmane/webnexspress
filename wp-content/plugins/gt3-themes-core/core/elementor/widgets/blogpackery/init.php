			...
			if ($settings) {
	            ...
	            if (!empty($wp_get_attachment_url)) {
		            if (function_exists('getSolidColorFromImage')) {
	        			...
	        			$featured_image .= '<div class="gt3_blog_packery__image-placeholder'.(!empty($gt3_featured_image_class) ? ' gt3_lazyload__placeholder' : '').'" style="padding-bottom:'.(100*$ration).'%;'.'background-color:#'.$mainColor.';"></div>';
		            }
		            if ($lazyload) {
